...
function(e) {
    ...
    A.prototype.page = 1,
    A.prototype.tableCheckZdy = function(){
        var a = this;
        try {
            var $table = $(a.elem[0]).find('table');
            ...
            $table.find('tbody tr').on('click', function (e) {
                ...
                if (da - date > 200) {
                    ...
                    if (a.config.cols[0][0].checkbox === true) {
                        var $div = $(e.target).parent('div'),
                            $b = $div.hasClass('layui-form-checkbox') ? $div : $table.find('tbody tr').eq(index).find("input[type='checkbox'][name='layTableCheckbox']").next('div').click();
                        $table.find('tbody').each(function () {
                            $b.hasClass('layui-form-checked') ? $(this).find('tr').eq(index).addClass('zdy-layui-table-click') : $(this).find('tr').eq(index).removeClass('zdy-layui-table-click');
                        });
                    } else if (a.config.cols[0][0].event === 'md_radio') {
                        var b = $table.find('tbody tr').eq(index).find("input[type='radio']").next('div');
                        $(e.target).parent('div').hasClass('layui-form-radio') || b.click();
                        $table.find('tbody').each(function () {
                            $(this).find('tr').removeClass('zdy-layui-table-click');
                            b.hasClass('layui-form-radioed') && $(this).find('tr').eq(index).addClass('zdy-layui-table-click');
                        });
                    }
                }
            });
    ...
    A.prototype.pullData = function(e, i) {
        ...
        if (a.startTime = (new Date).getTime(), n.url) {
            ...
            t.ajax({
                ...
                dataType: "json",
                success: function(t) {
                    return t[r.statusName] != r.statusCode ? (a.renderForm(), a.layMain.html('<div class="' + f + '">' + (t[r.msgName] || "返回的数据状态异常") + "</div>")) : (a.renderData(t, e, t[r.countName]),d(), n.time = (new Date).getTime() - a.startTime + " ms",i && l.close(i),void ("function" == typeof n.done && n.done(t, e, t[r.countName])), a.tableCheckZdy())
